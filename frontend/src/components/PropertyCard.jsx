  ...
  return (
    ...
    >
      ...
      <Link to={`/propiedad/${propiedad.id}`} className="relative aspect-[4/3] w-full overflow-hidden bg-gray-100 block">
        <div className="absolute top-4 left-4 z-20 bg-white/95 backdrop-blur-sm px-4 py-1.5 rounded-full text-xs font-bold tracking-wide text-jo-dark shadow-sm">{propiedad.tipo}</div>

        {/* Precio como badge flotante */}
        <div className="absolute top-4 right-4 z-30 bg-primary text-white px-3 py-1.5 rounded-full text-sm font-bold shadow-lg">{propiedad.precio}</div>

        {propiedad.destacada && (
          <div className="absolute bottom-4 left-4 z-20 bg-primary/90 text-white px-3 py-1 rounded-full text-xs font-semibold shadow-md">Destacada</div>
        )}

        <img src={propiedad.imagen} alt={propiedad.titulo} className="object-cover w-full h-full transform group-hover:scale-105 transition-transform duration-500 ease-in-out" />
      </Link>
      ...
      <div className="p-6 flex flex-col flex-grow">
        ...
        <div className="mt-auto flex items-center justify-end">
          <Link to={`/propiedad/${propiedad.id}`} className="inline-flex items-center justify-center gap-2 px-4 py-2 bg-white border border-jo-border rounded-md text-jo-dark hover:bg-primary hover:text-white transition-colors">
            Ver detalle
